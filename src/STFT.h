...
static const int MAX_RANDOM = 10'000'000;
static std::uniform_int_distribution<> distrib(0, MAX_RANDOM);

static float get_random(){
    float result =  (float)distrib(gen) / ((float) distrib.max() /2.f);
    result -= 1.f;
    return result;
}
...
public:
    ...
    void updateThreshold(const float new_value){
        threshold = new_value;
    }

    void updateBlur(const float new_value){
        blur = new_value;
    }

    void updateRandomPhase(const float new_value){
        randomize_phase = new_value;
    }
    ...
    void processBlock (AudioSampleBuffer& block)
    {
        ...
        random_value = noi::Tools::linearCrossfade(get_random(), random_value, noise_filtering);
                    analysis (channel);
                    modification();
                    synthesis (channel);
                }
